    ...
    >
      ...
      <nav className="flex-1 py-4 px-2 space-y-0.5">
        {navItems.map(({ to, label, icon }) => (
          ...
          >
            <span className="w-4 text-center text-base shrink-0">{icon}</span>
            {expanded && <span className="sidebar-label-fade">{label}</span>}
          </NavLink>
        ))}
      </nav>

      {footerNote && expanded && (
        <div className="sidebar-label-fade p-4 border-t border-surface3">
          <p className="text-ink3 text-xs leading-relaxed">{footerNote}</p>
        </div>
      )}
    </aside>
  )
